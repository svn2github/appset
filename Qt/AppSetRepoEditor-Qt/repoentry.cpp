...
RepoEntry::RepoEntry(QString name, bool active) :
    active(active), valid(true), name(name)
{
}

QString RepoEntry::getName() const{
    if(!nameFilter.isValid() || !isValid()) return "";
...
QString RepoEntry::toString() const{
    ...
    if(comments.isEmpty()) list << (active?name:commentString+name);
    else list << comments.join("\n") << name;
    ...
    }else list2 << detailsComments.join("\n") << details;
