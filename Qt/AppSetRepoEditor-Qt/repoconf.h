...
public:
    explicit RepoConf(const QString & filePath = "");
    ...
    const QString & getConfPath() const {return repoConfFilePath;}

    //Table Model
    int rowCount(const QModelIndex &parent = QModelIndex()) const {return count();};
    int columnCount(const QModelIndex &parent = QModelIndex()) const {return detailsExists()?3:2;};
...
public slots:    
    ...
    void moveUp(int i){if(entries.count() && i>0) {entries.swap(i,i-1);emit dataChanged(QModelIndex(),QModelIndex());}}
    void moveDown(int i){if(entries.count() && i<entries.count()-1) {entries.swap(i,i+1);emit dataChanged(QModelIndex(),QModelIndex());}}
