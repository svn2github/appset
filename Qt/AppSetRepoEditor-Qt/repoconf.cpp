...
RepoConf::RepoConf(const QString &filePath) {
    if(!filePath.isEmpty()) configure(filePath);
}
...
bool RepoConf::configure(const QString &filePath){
    QFile confFile(filePath);
    QTextStream confFileStream(&confFile);
    int status = 0;

    if(!confFile.exists() || !confFile.open(QIODevice::ReadOnly)) return false;

    while(!confFileStream.atEnd()){
        QStringList pair = confFileStream.readLine().split('=');

        if( pair.count() < 2 ) continue;

        QString key = pair.at(0), value = pair.at(1);

        if( key == "repo_conf_path" ) {repoConfFilePath = value;status++;}
        else if( key == "repo_match_regexp" ) {repoMatch = QRegExp(value);status++;}
        else if( key == "detail_match_regexp" ) {detailMatch = QRegExp(value);status++;}
        else if( key == "repo_name_filter" ) {RepoEntry::nameFilter = QRegExp(value);status++;}
        else if( key == "conf_comment_string" ) {commentString = value;status++;}
        else if( key == "repo_name_format" ) {RepoEntry::repoFormat = value;status++;}
    }

    confFile.close();

    return ( (status==6) && loadConf(repoConfFilePath) );
}
...
bool RepoConf::saveChanges(const QString & backup){
    if(!backup.isEmpty()){
        QMessageBox mbexists(QMessageBox::Warning,tr("Backup error"),tr("Backup file already exists.")+QString("\n")+tr("Do you want to overwrite it?"),QMessageBox::Yes|QMessageBox::No);
        if(QFile::exists(backup) && mbexists.exec()==QMessageBox::Yes){
            QFile::remove(backup);
        }
        QMessageBox mberror(QMessageBox::Critical,tr("Backup error"),tr("Can't create backup file.")+QString("\n")+tr("Do you want to proceed without a backup?"),QMessageBox::Yes|QMessageBox::No);
        if(!QFile::copy(repoConfFilePath,backup) && mberror.exec()==QMessageBox::No){
            return false;
        }
    }
    QFile confFile(repoConfFilePath);
    if(!confFile.open(QIODevice::WriteOnly))return false;
        ...
        case Qt::EditRole:
            switch(index.column()){
                case 0:
                    ...
                    break;
                case 1:
                    if(!matchRepo(RepoEntry::formatRepoName(value.toString()))) return false;
                ...
                case 2:
                    ...
                    for(int i=0;i<list.count();++i){
                        ...
                        if(!matchRepoDetails(list.at(i))) return false;
                    }
